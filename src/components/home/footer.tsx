    ...
    <footer id="faq" className="relative overflow-hidden border-t border-border bg-surface/60">
      ...
      <div className="relative mx-auto grid max-w-6xl gap-12 px-6 py-16 md:grid-cols-[1.5fr_repeat(4,1fr)]">
        <div>
          ...
          <Link
            href="#start"
            className="mt-6 inline-flex min-h-10 items-center gap-1.5 rounded-full bg-navy-mid px-4 text-sm font-semibold text-white transition hover:bg-navy"
          >
          ...
          <div key={column.title}>
            ...
            <ul className="space-y-3">
              {column.links.map(([label, href]) => (
                <li key={label}>
                  <Link href={href} className="text-sm text-muted-foreground transition-colors hover:text-navy-mid">
                    {label}
                  </Link>
                </li>
              ))}
